...
function resolveSelectedNameComputedProperties(context, selectedName, result) {
  if (isObject(selectedName)) {
    for (var property in selectedName) {
      result[property] = resolveSelectedNameComputedProperties(context, selectedName[property], result);
    }

  // if there is a list of values, then evaluate each object
  }else if (Array.isArray(selectedName)) {
    for (var p = 0; p < selectedName.length; p++) {
      result[p] = resolveSelectedNameComputedProperties(context, selectedName[p], result);
    }

  // if is a computed property then, eval that function passing the "context"
  }else if(typeof selectedName === 'function') {
    return selectedName(context);
  }
...
function templateResolve(config) {
    ...
    return function (name, context) {
        ...
        // enable computed properties on "is" value
        // creating a copy object, to avoid rule "caching", which has to be on the same type of the original object
        // basic evaluable types are "array" or "object"
        var result = isObject(selectedName) ? {} : [];
        resolveSelectedNameComputedProperties(context, selectedName, result);
...
function ruleEvaluate(spec, context, globalContext) {
    if (!Array.isArray(spec) && typeof spec !== 'function') {
        if (spec === context) {
            return true;
        }
        return false;
    ...
    // evaluate function return value (simple true:false evaluation)
    if (typeof spec === 'function') {
      return spec(context, globalContext);
    }
...
function fallbackOnLocalContext(config, context) {
    return Object.keys(config.when).every(function (key) {
        ...
        if (!val || !ruleEvaluate(config.when[key], val, context)) {
            return false;
        }
        return true;
    });
