...
function resolveSelectedNameComputedProperties(context, selectedName) {
  if (isObject(selectedName)) {
    for (var property in selectedName) {
      selectedName[property] = resolveSelectedNameComputedProperties(context, selectedName[property]);
    }
  ...
  // if there is a list of values, then evaluate each object
    ...
    for (var p = 0; p < selectedName.length; p++) {
      selectedName[p] = resolveSelectedNameComputedProperties(context, selectedName[p]);
    }
...
function templateResolve(config) {
    ...
    return function (name, context) {
        ...
        resolveSelectedNameComputedProperties(context, selectedName);

        // creating a copy for the response object
        return JSON.parse(JSON.stringify(selectedName));
    };
}
